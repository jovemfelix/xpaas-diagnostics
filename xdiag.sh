...
validatePodName() {
  ...
  if echo "$POD_DETAILS" | grep -q "Name:"; then
    return 0
  else
    return 1
  fi
...
# Parse arguments
while getopts ":cdfhjmto:p:" opt; do
  case $opt in
  c)
    GETCONFIG=1
    NOOP=1
...
if [ $GETCONFIG -eq 1 ]; then
  printf "\n  **** Retrieving standalone-openshift.xml ****\n\n"
  xstandaloneConfig "$PODNAME" "$OUTDIR"
fi

# Retrieve dockerfiles
if [ $GETDOCKERFILES -eq 1 ]; then
